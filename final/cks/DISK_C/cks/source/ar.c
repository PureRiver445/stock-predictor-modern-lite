#include "global.h"

int choose_p( float* data, int n, int max_p )
{
    float prev_aic = FLT_MAX;
    int result_p = 0;
    int p;
    float sigma2, aic;
    for( p = 1; p <= max_p; p++ )
    {
        float* r = (float*)safe_malloc( (p + 1) * sizeof(float) );
        float* phi = (float*)safe_malloc( p * sizeof(float) );

        calculate_r( data, n, r, p );

        sigma2 = levinson_durbin(r, phi, p);
        aic = 2 * p + n * log(sigma2);  

        if( aic < prev_aic )
        {
            result_p = p;
            prev_aic = aic;
        }
        
        #if defined( DEBUG )
            printf( "\np=%d,aic=%f\n", p, aic );
        #endif
        
        free( r );
        free( phi );
    }
    return result_p;
}

void pre_process( float* arr, int n, float* average, float* standard )
{
    float sum = 0.0f;
    int i;
    for (i = 0; i < n; i++ ) 
        sum += arr[i];
    
    *average = sum / n;
    for (i = 0; i < n; i++ ) 
        arr[i] -= *average;
    

    sum = 0.0f;
    for (i = 0; i < n; i++ ) 
        sum += arr[i] * arr[i];
    
    *standard = sqrt( sum / n );

    assert( *standard > 1e-6 );
    for (i = 0; i < n; i++ ) 
        arr[i] /= *standard;
    
}

void calculate_r( float* arr, int n, float* r, int p )
{
    int i, t;
    for( i = 0; i <= p; i++ )
    {
        r[i] = 0.0;
        for (t = 0; t < n - i; t++ ) {
            r[i] += arr[t] * arr[t + i];
        }
        r[i] /= n;
    }
}

float levinson_durbin( float* r, float* phi, int p)
{
    float* a = (float*)safe_malloc( (p + 1) * sizeof(float) );      
    float* prev_a = (float*)safe_malloc( (p + 1) * sizeof(float) ); 

    float epsilon = r[0];
    int m, j, i;
    a[0] = 1.0;

    for( m = 1; m <= p; m++)
    {
        float lambda = r[m];
        for( j = 1; j < m; j++) 
            lambda += a[j] * r[ m - j ];
        lambda = -lambda / epsilon;

        memmove( prev_a, a, m * sizeof(float) );

        a[m] = lambda;
        for( j = 1; j < m; j++) 
            a[j] = prev_a[j] + lambda * prev_a[ m - j ];
        
        epsilon *= ( 1 - lambda * lambda );
    }

    for( i = 1; i <= p; i++) 
        phi[ i - 1 ] = -a[i];

    free( a );
    free( prev_a );

    return epsilon;
}

void get_arr( const stock_data* stock , float* arr, int num )
{
    FILE* fp = open_file( stock->code, PRED, "rb" );
    fseek( fp, sizeof(float) * (stock->days - num), SEEK_SET );
    SAFE_READ( arr, sizeof(float), num, fp );
    close_file( fp, stock->code, ".pred" );
    
    #if defined( DEBUG )
        printf( "\nchoosed close:\n");
        print_array( arr, num, FLOAT );
    #endif
}

void save_close( const stock_data* stock )
{
    int i;
    day_price info;
    FILE* kdayfp = open_file( stock->code, KDAY, "rb" );
    FILE* predfp = open_file( stock->code, PRED, "wb" );
    float* close_buffer = (float*)safe_malloc( sizeof(float) * stock->days );
    for( i=0; i<stock->days; i++)
    {
        SAFE_READ( &info, sizeof(day_price), 1, kdayfp );
        close_buffer[i] = info.close;
    }
    #if defined( DEBUG )
        printf( "\nall close:\n");
        print_array( close_buffer, stock->days, FLOAT );
        printf( "\nall close done\n");
    #endif
    SAFE_WRITE( close_buffer, sizeof(float), stock->days, predfp );
    free( close_buffer );
    close_file( predfp, stock->code, ".pred" );
    close_file( kdayfp, stock->code, ".kday" );
}

void ar_pred( const stock_data* stock , int num )
{
    float* arr = (float*)safe_malloc( sizeof(float) * num );
    float average, standard;
    int p, i, j;
    float* r;
    float* phi;
    float* x;

    save_close( stock );
    get_arr( stock, arr , num);

    pre_process( arr, num, &average, &standard);

    p = choose_p( arr, num, 10 );
    r = (float*)safe_malloc( (p + 1) * sizeof(float) );
    phi = (float*)safe_malloc( p * sizeof(float) );

    calculate_r( arr, num, r, p );
    levinson_durbin( r, phi, p );

    x = (float*)safe_malloc( (p + FORECAST_DAYS) * sizeof(float) );  
    memmove( x, arr + num - p, p * sizeof(float) );
    free( arr );

    for( i = 0; i < FORECAST_DAYS; i++)
    {
        x[ i + p ] = 0.0;
        for(j = 0; j < p; j++)
            x[ i + p ] += phi[j] * x[ i + p - 1 - j];
    }
    for( i = 0; i < p + FORECAST_DAYS; i++)
        x[ i ] = x[ i ] * standard + average;
    
    #if defined(DEBUG)
        printf( "\nresult:\n");
        printf( "p: %d\n", p );
        printf( "r:");
        print_array( r, p + 1, FLOAT );
        printf( "\nphi:");
        print_array( phi, p, FLOAT );
        printf( "\npred:");
        print_array( x, p + FORECAST_DAYS, FLOAT );
    #endif
    
    save_pred( stock, x + p );

    free( r );
    free( phi );
    free( x );
}

void save_pred( const stock_data* stock , float* x )
{
    FILE* predfp = open_file( stock->code, PRED, "rb+" );
    fseek( predfp, stock->days * sizeof(float), SEEK_SET );
    SAFE_WRITE( x, sizeof(float), FORECAST_DAYS, predfp );
    close_file( predfp, stock->code, ".pred" );
}
