...
float predict(const individual_data* individual, const feature_data* feature)
{
    ...
    score += feature->trend * individual->weights[5];
    score += feature->wl * individual->weights[6];
...
#if defined(DEBUG)
    printf("\ngene result:\n");
    printf("weights: MA_cross: %.2f, EXPMA_cross: %.2f, XS_channel: %.2f, KDJ_status: %.2f, MACD_change: %.2f, close_trend: %.2f, wl_status: %.2f\n",
           best.weights[0], best.weights[1], best.weights[2], best.weights[3], best.weights[4], best.weights[5], best.weights[6]);
    printf("last day: trend: %d, MA_cross: %d, EXPMA_cross: %d, XS_channel: %d, KDJ_status: %d, MACD_change: %d, CLOSE_trend: %d, wl_status: %d\n",
           last.trend, last.ma, last.expma, last.xuechi, last.kdj, last.macd, last.trend, last.wl);
...
void macd_change(const stock_data* stock, feature_data* feature)
{
    ...
    macd_start = 3 * stock->days * sizeof(float) - TRAIN_DAYS * sizeof(float) - sizeof(float);
...
void ma_cross(const stock_data* stock, feature_data* feature)
{
    ...
    block_bytes = BLOCK_SIZE * sizeof(float);
    block_num = TRAIN_DAYS / BLOCK_SIZE;
    ma5_start = (stock->days - TRAIN_DAYS) * sizeof(float) - sizeof(float);
    ma10_start = 2 * stock->days * sizeof(float) - TRAIN_DAYS * sizeof(float) - sizeof(float);
...
void expma_cross(const stock_data* stock, feature_data* feature)
{
    ...
    block_bytes = BLOCK_SIZE * sizeof(float);
    block_num = TRAIN_DAYS / BLOCK_SIZE;
    expma12_start = (stock->days - TRAIN_DAYS) * sizeof(float) - sizeof(float);
    expma50_start = 2 * stock->days * sizeof(float) - TRAIN_DAYS * sizeof(float) - sizeof(float);
...
void xuechi_break_close(const stock_data* stock, feature_data* feature)
{
    ...
    close_start = up_start - sizeof(float);
...
#if defined(DEBUG)
            printf("%d %d ", feature[i * BLOCK_SIZE + j].xuechi, feature[i * BLOCK_SIZE + j].trend);
#endif
        }
    }
...
void wl_status(const stock_data* stock, feature_data* feature)
{
    FILE* wlfp;
    int block_bytes, block_num, start, i, j;
    float* buffer;

    wlfp = open_file(stock->code, WILLIAMS, "rb");
    block_bytes = BLOCK_SIZE * sizeof(float);
    block_num = TRAIN_DAYS / BLOCK_SIZE;
    start = (stock->days - TRAIN_DAYS) * sizeof(float);

    buffer = (float*)safe_malloc(block_bytes);

    for (i = 0; i < block_num; i++)
    {
        fseek(wlfp, start + i * block_bytes, SEEK_SET);
        SAFE_READ(buffer, sizeof(float), BLOCK_SIZE, wlfp);
        fseek(wlfp, start + i * block_bytes, SEEK_SET);
        SAFE_READ(buffer, sizeof(float), BLOCK_SIZE, wlfp);
        ...
        {
            if ( buffer[j] < -80 )
                feature[i * BLOCK_SIZE + j].wl = UP;
            else if ( buffer[j] > -20 )
                feature[i * BLOCK_SIZE + j].wl = DOWN;
            else
                feature[i * BLOCK_SIZE + j].wl = NONE;
        }
    }

    free(buffer);
    close_file(wlfp, stock->code, ".wl");
}